...
_Bool get_date (struct timespec *, char const *, struct timespec const *);
# 30 "touch.c" 2
# 1 "../lib/posixtm.h" 1
# 25 "../lib/posixtm.h"
# 1 "../lib/time.h" 1
...
touch (const char *file)
{
 ...
 {
   if (open_errno)
     error (0, open_errno, gettext ("creating %s"), quote (file));
   else
     {
       if (no_create && ((*__errno_location ()) == 2 || (*__errno_location ()) == 9))
  return 1;
  ...
  if (amtime_now)
    ...
    {
      timespec[0] = (change_times & 1
       ? newtime[0]
       : get_stat_atime (&sbuf));
      timespec[1] = (change_times & 2
       ? newtime[1]
       : get_stat_mtime (&sbuf));
      t = timespec;
    }

  ok = (gl_futimens (fd, (fd == 1 ? ((void *)0) : file), t) == 0);

  if (fd == 0)
    {
      if (close (0) != 0)
 {
   error (0, (*__errno_location ()), gettext ("closing %s"), quote (file));
   return 0;
 }
    }
  else if (fd == 1)
...
main (int argc, char **argv)
{
  ...
  while ((c = getopt_long (argc, argv, "acd:fmr:t:", longopts, ((void *)0))) != -1)
    {
      switch (c)
 {
 case 'a':
 ...
 case GETOPT_VERSION_CHAR: version_etc (stdout, "touch", "GNU coreutils", "6.10", "Paul Rubin", "Arnold Robbins, Jim Kingdon, David MacKenzie", "Randy Smith", (char *) ((void *)0)); exit (0); break;;
