...
usage (int status)
{
  if (status != 0)
  ...
  else
    {
      ...
      fputs_unlocked (gettext ("  -q, --quiet      suppress diagnostics about file/dir-creation failure\n"),stdout);


      fputs_unlocked (gettext ("  -u, --dry-run    do not create anything; merely print a name (unsafe)\n"),stdout);


      fputs_unlocked (gettext ("  --tmpdir[=DIR]   interpret TEMPLATE relative to DIR.  If DIR is\n                     not specified, use $TMPDIR if set, else /tmp.\n                     With this option, TEMPLATE must not be an absolute name.\n                     Unlike with -t, TEMPLATE may contain slashes, but even\n                     here, mktemp still creates only the final component.\n"),stdout);
      ...
      fputs_unlocked ("\n",stdout);
      fputs_unlocked (gettext ("  -p DIR           use DIR as a prefix; implies -t [deprecated]\n"),stdout);
...
static int
mkstemp_len (char *tmpl, size_t suff_len, _Bool dry_run)
{
  return gen_tempname_len (tmpl, dry_run ? 3 : 1, suff_len);
}

static int
mkdtemp_len (char *tmpl, size_t suff_len, _Bool dry_run)
{
  return gen_tempname_len (tmpl, dry_run ? 3 : 2, suff_len);
}
...
{
  char *dest_dir;
  char *dest_dir_arg = ((void *)0);
  _Bool suppress_stderr = 0;
  int c;
  unsigned int n_args;
  char *template;
  ...
  while ((c = getopt_long (argc, argv, "dp:qtuV", longopts, ((void *)0))) != -1)
    {
      switch (c)
        ...
        case 'V':
        case GETOPT_VERSION_CHAR: version_etc (stdout, "mktemp", "GNU coreutils", "6.10", "Jim Meyering", (char *) ((void *)0)); exit (0); break;;
        default:
          usage (1);
        }
    }
