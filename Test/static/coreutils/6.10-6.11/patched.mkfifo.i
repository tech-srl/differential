...
usage (int status)
{
  if (status != 0)
  ...
  else
    {
      ...
      fputs_unlocked (gettext ("  -Z, --context=CTX  set the SELinux security context of each NAME to CTX\n"),stdout);
...
main (int argc, char **argv)
{
 ...
 case GETOPT_VERSION_CHAR: version_etc (stdout, "mkfifo", "GNU coreutils", "6.11", "David MacKenzie", (char *) ((void *)0)); exit (0); break;;
 default:
   usage (1);
 }
    }
 ...
 error (1, 0, gettext ("invalid mode"));
      newmode = mode_adjust (newmode, 0, umask (0), change, ((void *)0));
      free (change);
