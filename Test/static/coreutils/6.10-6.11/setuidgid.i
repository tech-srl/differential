...
usage (int status)
{
  if (status != 0)
    ...
    {
      printf (gettext ("Usage: %s OPTION USER COMMAND [ARGUMENT]...\n  or:  %s OPTION\n"),



              program_name, program_name);

      fputs_unlocked (gettext ("Drop any supplemental groups, assume the user-ID and group-ID of the specified\nUSER (numeric ID or user name), and run COMMAND with any specified ARGUMENTs.\nExit with status 111 if unable to assume the required user and group ID.\nOtherwise, exit with the exit status of COMMAND.\nThis program is useful only when run by root (user ID zero).\n\n"),stdout);
      ...
      fputs_unlocked (gettext ("  -g GID[,GID1...]  also set the primary group-ID to the numeric GID, and\n                    (if specified) supplemental group IDs to GID1, ...\n"),stdout);
...
main (int argc, char **argv)
{
  ...
  initialize_exit_failure (111);
  atexit (close_stdout);

  parse_long_options (argc, argv, "setuidgid", "GNU coreutils", "6.10",
                      usage, "Jim Meyering", (char const *) ((void *)0));
  {
    int c;
    while ((c = getopt_long (argc, argv, "+g:", ((void *)0), ((void *)0))) != -1)
      {
        switch (c)
          {
            case 'g':
              {
                ...
                while (1)
                  {
                    if (! (xstrtoul (gr, &ptr, 10, &tmp_ul, ((void *)0)) == LONGINT_OK
                           && tmp_ul <= ((gid_t) (! (! ((gid_t) 0 < (gid_t) -1)) ? (gid_t) -1 : ~ (~ (gid_t) 0 << (sizeof (gid_t) * 8 - 1))))))
                      error (1, 0, gettext ("invalid group %s"),
                             quote (gr));
                    ...
                    if (*ptr != ',')
                      {
                        error (0, 0, gettext ("invalid group %s"), quote (gr));
                        usage (111);
                      }
                    gr = ptr + 1;
                  }
...

  {
    ...
    if (xstrtoul (user, &ptr, 10, &tmp_ul, "") == LONGINT_OK
        && tmp_ul <= ((uid_t) (! (! ((uid_t) 0 < (uid_t) -1)) ? (uid_t) -1 : ~ (~ (uid_t) 0 << (sizeof (uid_t) * 8 - 1)))))
      {
        uid = tmp_ul;
        have_uid = 1;
      }
    ...
    if (!have_uid)
      {
        ...
        if (pwd == ((void *)0))
          {
            error (111, (*__errno_location ()),
                   gettext ("unknown user-ID: %s"), quote (user));
            usage (111);
          }
        uid = pwd->pw_uid;
      }
    else if (n_gids == 0)
      {
        ...
        if (pwd == ((void *)0))
          {
            error (111, (*__errno_location ()),
                   gettext ("to use user-ID %s you need to use -g too"), quote (user));
            usage (111);
          }
      }
    ...
    if (n_gids == 0)
      {
        ...
        if (n_gids <= 0)
          error (1, (*__errno_location ()), gettext ("failed to get groups for user %s"),
                 quote (pwd->pw_name));
      }

    if (setgroups (n_gids, gids))
      error (111, (*__errno_location ()),
             gettext ("failed to set supplemental group(s)"));
  ...
  if (setgid (primary_gid))
    error (111, (*__errno_location ()),
           gettext ("cannot set group-ID to %lu"), (unsigned long int) primary_gid);

  if (setuid (uid))
    error (111, (*__errno_location ()),
           gettext ("cannot set user-ID to %lu"), (unsigned long int) uid);

  {
    char **cmd = argv + optind + 1;
    int exit_status;
    execvp (*cmd, cmd);
    exit_status = ((*__errno_location ()) == 2 ? EXIT_ENOENT : EXIT_CANNOT_INVOKE);

    error (0, (*__errno_location ()), gettext ("cannot run command %s"), quote (*cmd));
    exit (exit_status);
  }
}
