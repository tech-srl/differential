...
# 26 "remove.c" 2
# 1 "../lib/cycle-check.h" 1
# 25 "../lib/cycle-check.h"
# 1 "../lib/dev-ino.h" 1
...
# 26 "../lib/cycle-check.h" 2
# 1 "../lib/same-inode.h" 1
# 27 "../lib/cycle-check.h" 2
...
cache_fstatat (int fd, char const *file, struct stat *st, int flag)
{
  if (st->st_size == -1 && fstatat (fd, file, st, flag) != 0)
    st->st_size = -1 - (*__errno_location ());
  if (0 <= st->st_size)
    return 0;
  (*__errno_location ()) = -1 - st->st_size;
  return -1;
}
...
pop_dir (Dirstack_state *ds)
{
  ...
  ((n_lengths > 0) ? (void) (0) : __assert_fail ("n_lengths > 0", "remove.c", 266, __PRETTY_FUNCTION__));
  size_t top_len = length[n_lengths - 1];
  ((top_len >= 2) ? (void) (0) : __assert_fail ("top_len >= 2", "remove.c", 268, __PRETTY_FUNCTION__));


  ((__extension__ ({ struct obstack const *__o = (&ds->dir_stack); (unsigned) (__o->next_free - __o->object_base); }) >= top_len) ? (void) (0) : __assert_fail ("__extension__ ({ struct obstack const *__o = (&ds->dir_stack); (unsigned) (__o->next_free - __o->object_base); }) >= top_len", "remove.c", 271, __PRETTY_FUNCTION__));
  ...
  ((__extension__ ({ struct obstack const *__o = (&ds->len_stack); (unsigned) (__o->next_free - __o->object_base); }) >= sizeof (size_t)) ? (void) (0) : __assert_fail ("__extension__ ({ struct obstack const *__o = (&ds->len_stack); (unsigned) (__o->next_free - __o->object_base); }) >= sizeof (size_t)", "remove.c", 275, __PRETTY_FUNCTION__));
 ...
 return ((void *)0);
  ...
  if ((strcmp (filename, ".") == 0) && rm_dir_len)
  ...
  else
    {
      ...
      ((strlen (g_buf) + 1 == n_bytes_needed) ? (void) (0) : __assert_fail ("strlen (g_buf) + 1 == n_bytes_needed", "remove.c", 345, __PRETTY_FUNCTION__));
    }
...
{
  ((0 < AD_stack_height (ds)) ? (void) (0) : __assert_fail ("0 < AD_stack_height (ds)", "remove.c", 425, __PRETTY_FUNCTION__));
...
{
  __extension__ ({ struct obstack *__o = (&ds->dir_stack); void *__obj = (__extension__ ({ struct obstack *__o1 = (&ds->dir_stack); void *__value = (void *) __o1->object_base; if (__o1->next_free == __value) __o1->maybe_empty_object = 1; __o1->next_free = ((sizeof (int) < sizeof (void *) ? (__o1->object_base) : (char *) 0) + (((__o1->next_free) - (sizeof (int) < sizeof (void *) ? (__o1->object_base) : (char *) 0) + (__o1->alignment_mask)) & ~(__o1->alignment_mask))); if (__o1->next_free - (char *)__o1->chunk > __o1->chunk_limit - (char *)__o1->chunk) __o1->next_free = __o1->chunk_limit; __o1->object_base = __o1->next_free; __value; })); if (__obj > (void *)__o->chunk && __obj < (void *)__o->chunk_limit) __o->next_free = __o->object_base = (char *)__obj; else (obstack_free) (__o, __obj); });
...
# 519 "remove.c"
  ...
  ((AD_stack_height (ds)) ? (void) (0) : __assert_fail ("AD_stack_height (ds)", "remove.c", 525, __PRETTY_FUNCTION__));
  ...
  if (1 < AD_stack_height (ds))
    {
      ...
      if (closedir (dirp) != 0)
 {
   error (0, (*__errno_location ()), gettext ("FATAL: failed to close directory %s"),
 ...
 fd = openat (-100, xfull_filename (ds, "."), 00);
      ...
      if (fd < 0)
 {
   error (0, (*__errno_location ()), gettext ("FATAL: cannot open .. from %s"),
   quote (full_filename_ (ds, prev_dir)));
   goto next_cmdline_arg;
 }
      ...
      if (fstat (fd, &sb))
 {
   ...
   gettext ("FATAL: cannot ensure %s (returned to via ..) is safe"),
 ...
 {
   error (0, 0, gettext ("FATAL: directory %s changed dev/ino"),
   quote (full_filename_ (ds, ".")));
 close_and_next:;
   close (fd);
...
AD_mark_current_as_unremovable (Dirstack_state *ds)
{
  ...
  ((1 < AD_stack_height (ds)) ? (void) (0) : __assert_fail ("1 < AD_stack_height (ds)", "remove.c", 634, __PRETTY_FUNCTION__));
  ...
  struct stat const *dir_sb_from_parent)
{
 ...
 {
   error (0, (*__errno_location ()), gettext ("FATAL: cannot enter directory %s"),
   quote (full_filename_ (ds, ".")));
   longjmp (ds->current_arg_jumpbuf, 1);
 }
 ...
 {
   error (0, 0,
   gettext ("FATAL: just-changed-to directory %s changed dev/ino"),
   quote (full_filename_ (ds, ".")));
   longjmp (ds->current_arg_jumpbuf, 1);
 }
    }
 ...
 Ternary *is_empty)
{
      ...
      if (write_protected <= 0 && dirent_type == DT_UNKNOWN)
 {
  ...
  dirent_type = DT_DIR;
   ...
   else
     {
       ...
       write_protected = (*__errno_location ());
     }
 }
 ...
 {
   error (0, write_protected, gettext ("cannot remove %s"), quoted_name);
   return RM_ERROR;
 }
...
# 1080 "remove.c"
 ...
 {
   error (0, 21, gettext ("cannot remove %s"),
   ...
   )
 {
   ...
   return RM_ERROR;
      ...
      ((!cache_stat_ok (st) || ((((st->st_mode)) & 0170000) == (0040000))) ? (void) (0) : __assert_fail ("!cache_stat_ok (st) || ((((st->st_mode)) & 0170000) == (0040000))", "remove.c", 1129, __PRETTY_FUNCTION__));
    }
  else
    {
      ...
      struct rm_options const *x)
{
  ...
  ((((status) == RM_OK || (status) == RM_USER_DECLINED || (status) == RM_ERROR)) ? (void) (0) : __assert_fail ("((status) == RM_OK || (status) == RM_USER_DECLINED || (status) == RM_ERROR)", "remove.c", 1249, __PRETTY_FUNCTION__));
  *subdir = ((void *)0);
 ...
 case RM_NONEMPTY_DIR:
  ...
  if (ignorable_missing (x, (*__errno_location ())))
  ...
  else
    {
      ...
      if (unlinkat (dirfd (*dirp), f, 0x200) == 0)
        ...
        error (0, saved_errno,
        gettext ("cannot remove %s"), quote (full_filename_ (ds, f)));
    }
  ...
  break;
     ...
     *subdir = xstrdup (f);
     if (closedir (*dirp) != 0)
       {
  error (0, 0, gettext ("failed to close directory %s"),
         quote (full_filename_ (ds, ".")));
  status = RM_ERROR;
       }
     *dirp = subdir_dirp;
 ...
 break;
  ...
  ((*dirp != ((void *)0)) ? (void) (0) : __assert_fail ("*dirp != ((void *)0)", "remove.c", 1361, __PRETTY_FUNCTION__));
  ((0 <= fcntl (dirfd (*dirp), 1)) ? (void) (0) : __assert_fail ("0 <= fcntl (dirfd (*dirp), 1)", "remove.c", 1362, __PRETTY_FUNCTION__));
...
# 1400 "remove.c"
      ...
      else
 {
   ...
   error (0, saved_errno,
   gettext ("cannot remove %s"), quote (full_filename_ (ds, dir)));
 }
  ...
  if ((x->root_dev_ino && ((*dir_st).st_ino == (*x->root_dev_ino).st_ino && (*dir_st).st_dev == (*x->root_dev_ino).st_dev)))
    {
      do { if ((strcmp (full_filename_ (ds, dir), "/") == 0)) error (0, 0, gettext ("it is dangerous to operate recursively on %s"), quote (full_filename_ (ds, dir))); else error (0, 0, gettext ("it is dangerous to operate recursively on %s (same as %s)"), quote_n (0, full_filename_ (ds, dir)), quote_n (1, "/")); error (0, 0, gettext ("use --no-preserve-root to override this failsafe")); } while (0);
      status = RM_ERROR;
 ...
 dirp = ((void *)0);
 ((fd != -100 || AD_stack_height (ds) == 1) ? (void) (0) : __assert_fail ("fd != -100 || AD_stack_height (ds) == 1", "remove.c", 1483, __PRETTY_FUNCTION__));
 ...
 if (tmp_status == RM_OK)
  ...
  goto closedir_and_return;
     ...
     if (unlinkat (fd, empty_dir, 0x200) == 0)
       {
  if (x->verbose)
    printf (gettext ("removed directory: %s\n"),
     quote (full_filename_ (ds, empty_dir)));
       }
     else
       {
  error (0, (*__errno_location ()), gettext ("cannot remove directory %s"),
         quote (full_filename_ (ds, empty_dir)));
 ...
 if (dirp == ((void *)0))
   {
     error (0, (*__errno_location ()), gettext ("FATAL: cannot return to .. from %s"),
     quote (full_filename_ (ds, ".")));
     close (fd);
     longjmp (ds->current_arg_jumpbuf, 1);
   }
      ...
      struct rm_options const *x, int *cwd_errno)
{
  ...
  if (dot_or_dotdot (base))
    {
      error (0, 0, gettext (base == filename ? "cannot remove directory %s" : "cannot remove %s directory %s"),
      ...
      if (cache_fstatat (-100, filename, &st, 0x100) != 0)
 {
   if (ignorable_missing (x, (*__errno_location ())))
     return RM_OK;
   error (0, (*__errno_location ()), gettext ("cannot remove %s"), quote (filename));
   return RM_ERROR;
 }
      if (((st).st_ino == (*(x->root_dev_ino)).st_ino && (st).st_dev == (*(x->root_dev_ino)).st_dev))
 {
   error (0, 0, gettext ("cannot remove root directory %s"), quote (filename));
   return RM_ERROR;
 }
    }
...
rm (size_t n_files, char const *const *file, struct rm_options const *x)
{
 ...
 {
   error (0, 0, gettext ("cannot remove relative-named %s"), quote (file[i]));
   status = RM_ERROR;
 }
      else
 {
   enum RM_status s = rm_1 (ds, file[i], x, &cwd_errno);
   ((((s) == RM_OK || (s) == RM_USER_DECLINED || (s) == RM_ERROR)) ? (void) (0) : __assert_fail ("((s) == RM_OK || (s) == RM_USER_DECLINED || (s) == RM_ERROR)", "remove.c", 1629, __PRETTY_FUNCTION__));
  ...
  if (x->require_restore_cwd && cwd_errno)
    {
      error (0, cwd_errno,
      gettext ("cannot restore current working directory"));
      status = RM_ERROR;
    }
