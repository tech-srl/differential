...
swallow_file_in_memory (const char *file_name, BLOCK *block)
{
  ...
  if (using_stdin)
    ...
    if ((file_handle = open (file_name, 00)) < 0)
      error (1, (*__errno_location ()), "%s", file_name);
  ...
  if (((((stat_block.st_mode)) & 0170000) == (0100000)))
    {
...
# 559 "ptx.c"
    ...
    {
      block->start = xmalloc ((size_t) 1 << 12);
      used_length = 0;
      allocated_length = (1 << 12);
...
digest_word_file (const char *file_name, WORD_TABLE *table)
{
      ...
      if (cursor > word_start)
 {
   if (table->length == table->alloc)
     {
       if (((4294967295U) / sizeof *table->start - 1) / 2 < table->alloc)
  xalloc_die ();
...
# 1004 "ptx.c"
   ...
   if (number_of_occurs[0] == occurs_alloc[0])
     {
       if (((4294967295U) / sizeof *occurs_table[0] - 1) / 2
    < occurs_alloc[0])
  xalloc_die ();
...
# 1464 "ptx.c"
 ...
 = keyafter.start - (half_line_width + maximum_word_length);
  ...
  if (truncation_string)
    {
      cursor = before.start;
      while (cursor > text_buffer.start && ((*__ctype_b_loc ())[(int) ((to_uchar (cursor[-1])))] & (unsigned short int) _ISspace)) cursor--;
      before_truncation = cursor > left_context_start;
    }
  else
    before_truncation = 0;

  while (before.start < text_buffer.end && ((*__ctype_b_loc ())[(int) ((to_uchar (*before.start)))] & (unsigned short int) _ISspace)) before.start++;
# 1504 "ptx.c"
  tail_max_width
  ...
  if (tail_max_width > 0)
    {
      ...
      while (tail.start < text_buffer.end && ((*__ctype_b_loc ())[(int) ((to_uchar (*tail.start)))] & (unsigned short int) _ISspace)) tail.start++;
  ...
  if (head_max_width > 0)
    {
      ...
      while (head.end > text_buffer.start && ((*__ctype_b_loc ())[(int) ((to_uchar (head.end[-1])))] & (unsigned short int) _ISspace)) head.end--;
...
main (int argc, char **argv)
{
 ...
 case GETOPT_VERSION_CHAR: version_etc (stdout, "ptx", "GNU coreutils", "6.10", gettext ("F. Pinard"), (char *) ((void *)0)); exit (0); break;;
 }
    }
