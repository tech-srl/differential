...
static uintmax_t gobble_file (char const *name, enum filetype type,
         ...
         char const *dirname);
static _Bool print_color_indicator (const char *name, mode_t mode, int linkok,
...
dev_ino_pop (void)
{
  ((sizeof (struct dev_ino) <= __extension__ ({ struct obstack const *__o = (&dev_ino_obstack); (unsigned) (__o->next_free - __o->object_base); })) ? (void) (0) : __assert_fail ("sizeof (struct dev_ino) <= __extension__ ({ struct obstack const *__o = (&dev_ino_obstack); (unsigned) (__o->next_free - __o->object_base); })", "ls.c", 925, __PRETTY_FUNCTION__));
...
main (int argc, char **argv)
{
  ...
  (((sizeof color_indicator / sizeof *(color_indicator)) + 1 == (sizeof indicator_name / sizeof *(indicator_name))) ? (void) (0) : __assert_fail ("(sizeof color_indicator / sizeof *(color_indicator)) + 1 == (sizeof indicator_name / sizeof *(indicator_name))", "ls.c", 1160, __PRETTY_FUNCTION__));

  exit_status = 0;
  print_dir_name = 1;
  pending_dirs = ((void *)0);

  current_time.tv_sec = ((time_t) (! (! ((time_t) 0 < (time_t) -1)) ? (time_t) 0 : ((time_t) ~ (time_t) 0 < (time_t) -1) ? ~ (time_t) 0 : ~ (time_t) 0 << (sizeof (time_t) * 8 - 1)));
   ...
   if (thispend->name == ((void *)0))
     {
       ...
       ((found) ? (void) (0) : __assert_fail ("found", "ls.c", 1319, __PRETTY_FUNCTION__));
       dev_ino_free (found);
...
# 1356 "ls.c"
 ...
 raise (j);
  ...
  if ((!!active_dir_set))
    {
      ((hash_get_n_entries (active_dir_set) == 0) ? (void) (0) : __assert_fail ("hash_get_n_entries (active_dir_set) == 0", "ls.c", 1374, __PRETTY_FUNCTION__));
...
decode_switches (int argc, char **argv)
{
 ...
 case GETOPT_VERSION_CHAR: version_etc (stdout, (ls_mode == 1 ? "ls" : (ls_mode == 2 ? "dir" : "vdir")), "GNU coreutils", "6.11", "Richard Stallman", "David MacKenzie", (char *) ((void *)0)); exit (0); break;;
      ...
      _Bool command_line_arg, char const *dirname)
{
  ...
  ((! command_line_arg || inode == NOT_AN_INODE_NUMBER) ? (void) (0) : __assert_fail ("! command_line_arg || inode == NOT_AN_INODE_NUMBER", "ls.c", 2561, __PRETTY_FUNCTION__));
...
sort_files (void)
{
  ...
  if (! _setjmp (failed_strcoll))
    ...
    {
      use_strcmp = 1;
      ((sort_type != sort_version) ? (void) (0) : __assert_fail ("sort_type != sort_version", "ls.c", 3233, __PRETTY_FUNCTION__));
      initialize_ordering_vector ();
    }
