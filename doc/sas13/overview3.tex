...
Consider the two versions of a program for computing sign in~\figref{SignExample}, inspired by an example from~\cite{MauborgneRival07}. For these programs, we would like to establish that the output of $sign$ and $sign'$ only differs in the case where $x=0$ and that the difference is $sgn = 1 \neq sgn' = 0$ as generally described in \figref{SignSummary}.
