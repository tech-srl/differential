...
digest_check (const char *checkfile_name)
{
  ...
  do
    {
      ...
      ++line_number;
      if (line_number == 0)
 error (1, 0, gettext ("%s: too many checksum lines"),
        checkfile_name);
      ...
      && hex_digits (hex_digest)))
 {
   if (warn)
     {
       error (0, 0,
       gettext ("%s: %" "ll" "u" ": improperly formatted %s checksum line"),
      ...
      else
 {
  ...
  {
    printf ("%s: %s\n", filename,
     (cnt != digest_bin_bytes ? gettext ("FAILED") : gettext ("OK")));
    fflush_unlocked (stdout);
  }
     }
 }
      ...
      if (!status_only)
 {
   if (n_open_or_read_failures != 0)
     error (0, 0,
     ngettext ("WARNING: %" "ll" "u" " of %" "ll" "u"
        " listed file could not be read",
        "WARNING: %" "ll" "u" " of %" "ll" "u"
...
main (int argc, char **argv)
{
  ...
  while ((opt = getopt_long (argc, argv, "bctw", long_options, ((void *)0))) != -1)
    switch (opt)
      {
      case 'b':
 binary = 1;
 ...
 break;
      ...
      case GETOPT_VERSION_CHAR: version_etc (stdout, "sha512sum", "GNU coreutils", "6.10", "Ulrich Drepper", "Scott Miller", "David Madore", (char *) ((void *)0)); exit (0); break;;
      default:
 usage (1);
      }

  min_digest_line_length = ((512 / 4) + 2 + 1 );
