...
# 1 "/usr/include/errno.h" 1 3 4
# 5 "../lib/selinux/context.h" 2
# 15 "../lib/selinux/context.h"
...
static int
compute_context_from_mask (char* context, context_t *ret)
{
  _Bool ok = 1;
...
usage (int status)
{
 ...
 program_name, program_name, program_name);
      fputs_unlocked (gettext ("Change the security context of each FILE to CONTEXT.\nWith --reference, change the security context of each FILE to that of RFILE.\n\n  -c, --changes          like verbose but report only when a change is made\n  -h, --no-dereference   affect symbolic links instead of any referenced file\n                         (available only on systems with lchown system call)\n"),stdout);
      ...
      fputs_unlocked (gettext ("The following options modify how a hierarchy is traversed when the -R\noption is also specified.  If more than one is specified, only the final\none takes effect.\n\n  -H                     if a command line argument is a symbolic link\n                         to a directory, traverse it\n  -L                     traverse every symbolic link to a directory\n                         encountered\n  -P                     do not traverse any symbolic links (default)\n\n"),stdout);
# 403 "chcon.c"
...
main (int argc, char **argv)
{
  ...
  while ((optc = getopt_long (argc, argv, "HLPRchvu:r:t:l:", long_options, ((void *)0)))
  != -1)
    {
      switch (optc)
 {
 ...
 case GETOPT_VERSION_CHAR: version_etc (stdout, "chcon", "GNU coreutils", "6.10", "Russell Coker", "Jim Meyering", (char *) ((void *)0)); exit (0); break;;
 default:
   usage (1);
 }
    }
 ...
 error (1, (*__errno_location ()), gettext ("failed to get security context of %s"),
  ...
  else
    {
      ...
      context = context_new (specified_context);
      if (!context)
 error (1, 0, gettext ("invalid context: %s"),
