...
usage (int status)
{
  if (status != 0)
    ...
    fputs_unlocked (gettext ("Output fixed-size pieces of INPUT to PREFIXaa, PREFIXab, ...; default\nsize is 1000 lines, and default PREFIX is `x'.  With no INPUT, or when INPUT\nis -, read standard input.\n\n"),stdout);
      ...
      fprintf (stdout, gettext ("  -a, --suffix-length=N   use suffixes of length N (default %d)\n  -b, --bytes=SIZE        put SIZE bytes per output file\n  -C, --line-bytes=SIZE   put at most SIZE bytes of lines per output file\n  -d, --numeric-suffixes  use numeric suffixes instead of alphabetic\n  -l, --lines=NUMBER      put NUMBER lines per output file\n"), 2);
      ...
      fputs_unlocked (gettext ("      --verbose           print a diagnostic to standard error just\n                            before each output file is opened\n"),stdout);
...
cwrite (_Bool new_file_flag, const char *bp, size_t bytes)
{
 ...
 error (1, (*__errno_location ()), "%s", outfile);
      ...
      next_file_name ();
      if (verbose)
 fprintf (stderr, gettext ("creating file %s\n"), quote (outfile));
      output_desc = open_safer (outfile,
     01 | 0100 | 01000 | 0,
...
bytes_split (uintmax_t n_bytes, char *buf, size_t bufsize)
{
  ...
  do
    {
      n_read = full_read (0, buf, bufsize);
      if (n_read == ((size_t) -1))
  ...
  do
    {
      ...
      n_read = full_read (0, buf + n_buffered, n_bytes - n_buffered);
      if (n_read == ((size_t) -1))
 error (1, (*__errno_location ()), "%s", infile);
...
main (int argc, char **argv)
{
  ...
  while (1)
    {
      ...
      c = getopt_long (argc, argv, "0123456789C:a:b:dl:", longopts, ((void *)0));
      if (c == -1)
 break;
 ...
 case 'a':
   {
     unsigned long tmp;
     if (xstrtoul (optarg, ((void *)0), 10, &tmp, "") != LONGINT_OK
  || (4294967295U) / sizeof (size_t) < tmp)
       {
  error (0, 0, gettext ("%s: invalid suffix length"), optarg);
  usage (1);
       }
     suffix_length = tmp;
   }
 ...
 case 'b':
   ...
   if (xstrtoumax (optarg, ((void *)0), 10, &n_units, multipliers) != LONGINT_OK
       || n_units == 0)
     {
       error (0, 0, gettext ("%s: invalid number of bytes"), optarg);
       usage (1);
     }
   break;
 ...
 case 'l':
   ...
   if (xstrtoumax (optarg, ((void *)0), 10, &n_units, "") != LONGINT_OK
       || n_units == 0)
     {
       error (0, 0, gettext ("%s: invalid number of lines"), optarg);
       usage (1);
     }
   break;
 ...
 case '9':
   ...
   if (!( (void) (&(n_units) == (uintmax_t *) ((void *)0)), (void) (!!sizeof (struct { unsigned int verify_error_if_negative_size__: (! (! ((uintmax_t) 0 < (uintmax_t) -1))) ? 1 : -1; })), (void) (!!sizeof (struct { unsigned int verify_error_if_negative_size__: (sizeof (n_units) == sizeof (uintmax_t)) ? 1 : -1; })), (((uintmax_t) -1 / 10 < (n_units) || (uintmax_t) ((n_units) * 10 + (c - '0')) < (n_units)) ? 0 : (((n_units) = (n_units) * 10 + (c - '0')), 1)) ))
     {
       char buffer[(((sizeof (uintmax_t) * 8 - (! ((__typeof__ (uintmax_t)) 0 < (__typeof__ (uintmax_t)) -1))) * 146 / 485 + (! ((__typeof__ (uintmax_t)) 0 < (__typeof__ (uintmax_t)) -1)) + 1) + 1)];
       error (1, 0,
 ...
 case GETOPT_VERSION_CHAR: version_etc (stdout, "split", "GNU coreutils", "6.10", "Torbjorn Granlund", "Richard M. Stallman", (char *) ((void *)0)); exit (0); break;;
 ...
 default:
  ...
  if (n_units == 0)
    {
      error (0, 0, gettext ("invalid number of lines: 0"));
      usage (1);
    }
  ...
  if (fstat (0, &stat_buf) != 0)
    error (1, (*__errno_location ()), "%s", infile);
  in_blk_size = ((0 < (stat_buf).st_blksize && (stat_buf).st_blksize <= (4294967295U) / 8 + 1) ? (stat_buf).st_blksize : 512);
