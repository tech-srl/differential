...
{
  char buf[(((sizeof (intmax_t) * 8 - (! ((__typeof__ (intmax_t)) 0 < (__typeof__ (intmax_t)) -1))) * 146 / 485 + (! ((__typeof__ (intmax_t)) 0 < (__typeof__ (intmax_t)) -1)) + 1) + 1)];
   ...
   char const *user, char const *group)
{
      ...
      else
 {
  ...
  switch (changed)
    {
    case CH_SUCCEEDED:
      fmt = (user ? gettext ("changed ownership of %s to %s\n")
      : group ? gettext ("changed group of %s to %s\n")
