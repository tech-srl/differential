...
_Bool print_group_list (const char *, uid_t, gid_t, gid_t, _Bool);
# 33 "id.c" 2
...
usage (int status)
{
  if (status != 0)
  ...
  else
    {
      ...
      fputs_unlocked (gettext ("Print information for USERNAME, or the current user.\n\n  -a              ignore, for compatibility with other versions\n  -Z, --context   print only the security context of the current user\n  -g, --group     print only the effective group ID\n  -G, --groups    print all group IDs\n  -n, --name      print a name instead of a number, for -ugG\n  -r, --real      print the real ID instead of the effective ID, with -ugG\n  -u, --user      print only the effective user ID\n"),stdout);
# 95 "id.c"
...
main (int argc, char **argv)
{
  ...
  while ((optc = getopt_long (argc, argv, "agnruGZ", longopts, ((void *)0))) != -1)
    {
      switch (optc)
 {
 case 'a':
 ...
 case GETOPT_VERSION_CHAR: version_etc (stdout, "id", "GNU coreutils", "6.11", "Arnold Robbins", "David MacKenzie", (char *) ((void *)0)); exit (0); break;;
 default:
   usage (1);
 }
    }
  ...
  if (just_context && !selinux_enabled)
    error (1, 0, gettext ("cannot display context when selinux not enabled or when displaying the id\nof a different user"));
 ...
 error (1, 0, gettext ("%s: No such user"), argv[optind]);
  ...
  else
    {
      ...
      euid = geteuid ();
      pwd = getpwuid (euid);
      user_name = pwd ? xstrdup (pwd->pw_name) : ((void *)0);
      ruid = getuid ();
      egid = getegid ();
      rgid = getgid ();
    }
  ...
  if (use_name)
    {
      ...
      if (pwd == ((void *)0))
 {
   error (0, 0, gettext ("cannot find name for user ID %lu"),
