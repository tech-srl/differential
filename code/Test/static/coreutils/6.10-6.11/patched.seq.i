...
# 23 "../lib/math.h" 2
# 24 "seq.c" 2
# 1 "/usr/lib/gcc/i486-linux-gnu/4.4.3/include/float.h" 1 3 4
# 25 "seq.c" 2
...
scan_arg (const char *arg)
{
  ...
  while (((*__ctype_b_loc ())[(int) ((*arg))] & (unsigned short int) _ISspace) || *arg == '+')
    arg++;

  ret.width = strlen (arg);
  ret.precision = 2147483647;

  if (! arg[strcspn (arg, "xX")] && (sizeof (ret.value) == sizeof (float) ? __finitef (ret.value) : sizeof (ret.value) == sizeof (double) ? __finite (ret.value) : __finitel (ret.value)))
    {
...
validate_format (char const *fmt)
{
      ...
      if (p[0] == '%' && p[1] != '%' && p[1] != '\0')
 {
  ...
  if (n_directives == 0)
    {
      error (0, 0, gettext ("no %% directive in format string %s"), quote (fmt));
      usage (1);
    }
  else if (1 < n_directives)
    error (1, 0, gettext ("too many %% directives in format string %s"),
    quote (fmt));
}
   ...
   if (out_of_range)
     {
       ...
       long double x_val;
       char *x_str;
       int x_strlen = rpl_asprintf (&x_str, fmt, x);
       if (x_strlen < 0)
  xalloc_die ();
       x_str[x_strlen - layout.suffix_len] = '\0';

       if (xstrtold (x_str + layout.prefix_len, ((void *)0), &x_val, c_strtold)
    && abs_rel_diff (x_val, last) < 2.2204460492503131e-16)
  {
    char *x0_str = ((void *)0);
...
{
  static char format_buf[sizeof "%0.Lf" + 2 * ((sizeof (int) * 8 - (! ((__typeof__ (int)) 0 < (__typeof__ (int)) -1))) * 146 / 485 + (! ((__typeof__ (int)) 0 < (__typeof__ (int)) -1)) + 1)];
...
main (int argc, char **argv)
{
 ...
 case GETOPT_VERSION_CHAR: version_etc (stdout, "seq", "GNU coreutils", "6.11", "Ulrich Drepper", (char *) ((void *)0)); exit (0); break;;
  ...
  if (format_str)
    {
      ...
      if (! f)
 {
   error (0, 0, gettext ("invalid format string: %s"), quote (format_str));
   usage (1);
 }
      format_str = f;
    }
