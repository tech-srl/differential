...
usage (int status)
{
  if (status != 0)
  ...
  else
    {
      ...
      fputs_unlocked (gettext ("  \\a      alert (BEL)\n  \\b      backspace\n  \\c      produce no further output\n  \\f      form feed\n"),stdout);
      ...
      fputs_unlocked (gettext ("  \\xHH    byte with hexadecimal value HH (1 to 2 digits)\n  \\uHHHH  Unicode (ISO/IEC 10646) character with hex value HHHH (4 digits)\n  \\UHHHHHHHH  Unicode character with hex value HHHHHHHH (8 digits)\n"),stdout);
...
# 188 "printf.c"
static intmax_t vstrtoimax (char const *s) { char *end; intmax_t val; if (*s == '\"' || *s == '\'') { unsigned char ch = *++s; val = ch; if (*++s != 0 && !posixly_correct) error (0, 0, gettext (cfcc_msg), s); } else { (*__errno_location ()) = 0; val = (strtoimax (s, &end, 0)); verify_numeric (s, end); } return val; }
static uintmax_t vstrtoumax (char const *s) { char *end; uintmax_t val; if (*s == '\"' || *s == '\'') { unsigned char ch = *++s; val = ch; if (*++s != 0 && !posixly_correct) error (0, 0, gettext (cfcc_msg), s); } else { (*__errno_location ()) = 0; val = (strtoumax (s, &end, 0)); verify_numeric (s, end); } return val; }
static long double vstrtold (char const *s) { char *end; long double val; if (*s == '\"' || *s == '\'') { unsigned char ch = *++s; val = ch; if (*++s != 0 && !posixly_correct) error (0, 0, gettext (cfcc_msg), s); } else { (*__errno_location ()) = 0; val = (c_strtold (s, &end)); verify_numeric (s, end); } return val; }
      ...
      char const *argument)
{
 ...
 length_modifier = "ll" "d";
 length_modifier_len = sizeof "ll" "d" - 2;
 break;
...
main (int argc, char **argv)
{
  ...
  parse_long_options (argc, argv, "printf", "GNU coreutils", "6.10",
