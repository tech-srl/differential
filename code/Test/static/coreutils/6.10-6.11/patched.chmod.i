...
enum { I_RING_SIZE = 4 };
extern int (* verify_function__ (void)) [(!!sizeof (struct { unsigned int verify_error_if_negative_size__: (1 <= I_RING_SIZE) ? 1 : -1; }))];
# 31 "../lib/i-ring.h"
struct I_ring
{
...
mode_changed (char const *file, mode_t old_mode, mode_t new_mode)
{
 ...
 {
   if (!force_silent)
     error (0, (*__errno_location ()), gettext ("getting new attributes of %s"), quote (file));
   return 0;
 }
   ...
   enum Change_status changed)
{
  ...
  switch (changed)
    {
    case CH_SUCCEEDED:
      fmt = gettext ("mode of %s changed to %04lo (%s)\n");
      break;
    case CH_FAILED:
      fmt = gettext ("failed to change mode of %s to %04lo (%s)\n");
      break;
...
process_file (FTS *fts, FTSENT *ent)
{
      ...
      if (ent->fts_level == 0 && ent->fts_number == 0)
 {
   ...
   return 1;
    ...
    case 7:
      error (0, ent->fts_errno, gettext ("%s"), quote (file_full_name));
      ok = 0;
      break;
...
usage (int status)
{
  if (status != 0)
  ...
  else
    {
      ...
      fputs_unlocked (gettext ("  -f, --silent, --quiet   suppress most error messages\n  -v, --verbose           output a diagnostic for every file processed\n      --reference=RFILE   use RFILE's mode instead of MODE values\n  -R, --recursive         change files and directories recursively\n"),stdout);
...
main (int argc, char **argv)
{
 ...
 case '=':
  ...
  mode = ((void) (!!sizeof (struct { unsigned int verify_error_if_negative_size__: (sizeof *(mode) == 1) ? 1 : -1; })), x2realloc (mode, &mode_alloc));
       }
     mode[mode_len] = ',';
 ...
 case GETOPT_VERSION_CHAR: version_etc (stdout, "chmod", "GNU coreutils", "6.11", "David MacKenzie", "Jim Meyering", (char *) ((void *)0)); exit (0); break;;
 default:
   usage (1);
 }
    }
 ...
 error (1, (*__errno_location ()), gettext ("failed to get attributes of %s"),
  ...
  ok = process_files (argv + optind, 0x0001 | 0x0010);
