...
remove_parents (char *dir)
{
 ...
 --slash;
      ...
      if (verbose)
 error (0, 0, gettext ("removing directory, %s"), quote (dir));
      ...
      if (!ok)
 {
   ...
   if (ignore_fail_on_non_empty
       && errno_rmdir_non_empty ((*__errno_location ())))
     {
       ok = 1;
     }
   else
...
main (int argc, char **argv)
{
 ...
 case GETOPT_VERSION_CHAR: version_etc (stdout, "rmdir", "GNU coreutils", "6.10", "David MacKenzie", (char *) ((void *)0)); exit (0); break;;
 default:
   usage (1);
 }
    }
  ...
  for (; optind < argc; ++optind)
    {
      ...
      if (verbose)
 error (0, 0, gettext ("removing directory, %s"), dir);
