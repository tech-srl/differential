 ...
 *new++ = '\0';
      for (entry = table; ; entry++)
 {
   if (! entry->symbol[0])
     {
       error (0, 0, gettext (error_msgid), quote (str));
       usage (1);
     }
   if ((strcmp (entry->symbol, str) == 0))
     {
       if (! entry->value)
  error (1, 0, gettext (error_msgid), quote (str));
       value |= entry->value;
       break;
     }
 }
...
scanargs (int argc, char **argv)
{
      ...
      else
 {
   ...
   if ((strcmp (name, "ibs") == 0))
     ...
     {
       error (0, 0, gettext ("unrecognized operand %s=%s"),
...
main (int argc, char **argv)
{
  ...
  parse_long_options (argc, argv, "dd", "coreutils", "6.10",
