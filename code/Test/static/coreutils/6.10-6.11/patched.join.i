...
static size_t join_field_1 = (4294967295U);
static size_t join_field_2 = (4294967295U);
...
extract_field (struct line *line, char *field, size_t len)
{
  if (line->nfields >= line->nfields_allocated)
    {
      line->fields = ((void) (!!sizeof (struct { unsigned int verify_error_if_negative_size__: (sizeof *(line->fields) != 1) ? 1 : -1; })), x2nrealloc (line->fields, &line->nfields_allocated, sizeof *(line->fields)));
    }
...
free_prevline (void)
{
  ...
  for (i = 0; i < (sizeof (prevline) / sizeof *(prevline)); i++)
    {
      if (prevline[i])
 freeline (prevline[i]);
      free (prevline[i]);
...
getseq (FILE *fp, struct seq *seq, int whichfile)
{
  if (seq->count == seq->alloc)
    seq->lines = ((void) (!!sizeof (struct { unsigned int verify_error_if_negative_size__: (sizeof *(seq->lines) != 1) ? 1 : -1; })), x2nrealloc (seq->lines, &seq->alloc, sizeof *(seq->lines)));
...
add_field (int file, size_t field)
{
  ...
  ((file == 0 || file == 1 || file == 2) ? (void) (0) : __assert_fail ("file == 0 || file == 1 || file == 2", "join.c", 740, __PRETTY_FUNCTION__));
  ((file != 0 || field == 0) ? (void) (0) : __assert_fail ("file != 0 || field == 0", "join.c", 741, __PRETTY_FUNCTION__));
...
string_to_join_field (char const *str)
{
  ...
  extern int (* verify_function__ (void)) [(!!sizeof (struct { unsigned int verify_error_if_negative_size__: ((4294967295U) <= (2147483647L * 2UL + 1UL)) ? 1 : -1; }))];

  strtol_error s_err = xstrtoul (str, ((void *)0), 10, &val, "");
  if (s_err == LONGINT_OVERFLOW || (s_err == LONGINT_OK && (4294967295U) < val))
    val = (4294967295U);
...
main (int argc, char **argv)
{
 ...
 case GETOPT_VERSION_CHAR: version_etc (stdout, "join", "GNU coreutils", "6.11", "Mike Haertel", (char *) ((void *)0)); exit (0); break;;
 ...
 set_join_field (&join_field_2, i);
  ...
  if (join_field_1 == (4294967295U))
    join_field_1 = 0;
  if (join_field_2 == (4294967295U))
    join_field_2 = 0;

  fp1 = (strcmp (names[0], "-") == 0) ? stdin : fopen_safer (names[0], "r");
  if (!fp1)
    error (1, (*__errno_location ()), "%s", names[0]);
  fp2 = (strcmp (names[1], "-") == 0) ? stdin : fopen_safer (names[1], "r");
  if (!fp2)
    error (1, (*__errno_location ()), "%s", names[1]);
  if (fp1 == fp2)
