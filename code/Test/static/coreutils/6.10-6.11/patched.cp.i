...
extern int (* verify_function__ (void)) [(!!sizeof (struct { unsigned int verify_error_if_negative_size__: ((sizeof (sparse_type_string) / sizeof *(sparse_type_string)) == (sizeof (sparse_type) / sizeof *(sparse_type)) + 1) ? 1 : -1; }))];
...
extern int (* verify_function__ (void)) [(!!sizeof (struct { unsigned int verify_error_if_negative_size__: ((sizeof (reply_args) / sizeof *(reply_args)) == (sizeof (reply_vals) / sizeof *(reply_vals)) + 1) ? 1 : -1; }))];
...
usage (int status)
{
  if (status != 0)
  ...
  else
    {
      ...
      fputs_unlocked (gettext ("  -a, --archive                same as -dpR\n      --backup[=CONTROL]       make a backup of each existing destination file\n  -b                           like --backup but does not accept an argument\n      --copy-contents          copy contents of special files when recursive\n  -d                           same as --no-dereference --preserve=links\n"),stdout);
      ...
      fputs_unlocked (gettext ("  -l, --link                   link files instead of copying\n  -L, --dereference            always follow symbolic links in SOURCE\n"),stdout);



      fputs_unlocked (gettext ("  -P, --no-dereference         never follow symbolic links in SOURCE\n"),stdout);
      ...
      fputs_unlocked (gettext ("      --sparse=WHEN            control creation of sparse files\n      --strip-trailing-slashes  remove any trailing slashes from each SOURCE\n                                 argument\n"),stdout);
...
# 240 "cp.c"
      fputs_unlocked (gettext ("The backup suffix is `~', unless set with --suffix or SIMPLE_BACKUP_SUFFIX.\nThe version control method may be selected via the --backup option or through\nthe VERSION_CONTROL environment variable.  Here are the values:\n\n"),stdout);
     ...
     struct dir_attr *attr_list, const struct cp_options *x)
{
  ...
  do { dst_name = (__extension__ ({ __const char *__old = (const_dst_name); size_t __len = strlen (__old) + 1; char *__new = (char *) __builtin_alloca (__len); (char *) memcpy (__new, __old, __len); })); } while (0);
     ...
     const struct cp_options *x)
{
  ...
  do { dir = (__extension__ ({ __const char *__old = (const_dir); size_t __len = strlen (__old) + 1; char *__new = (char *) __builtin_alloca (__len); (char *) memcpy (__new, __old, __len); })); } while (0);
...
target_directory_operand (char const *file, struct stat *st, _Bool *new_dst)
{
  ...
  if (err)
    {
      if (err != 2)
 error (1, err, gettext ("accessing %s"), quote (file));
      *new_dst = 1;
    }
  return is_a_dir;
}
   ...
   if (parents_option)
     {
       ...
       do { arg_no_trailing_slash = (__extension__ ({ __const char *__old = (arg); size_t __len = strlen (__old) + 1; char *__new = (char *) __builtin_alloca (__len); (char *) memcpy (__new, __old, __len); })); } while (0);
   ...
   (dst_name, arg_in_concat - dst_name,
    ...
    &attr_list, &new_dst, x));
   ...
   else
     {
       ...
       do { char *tmp_abns_; do { tmp_abns_ = (__extension__ ({ __const char *__old = ((arg)); size_t __len = strlen (__old) + 1; char *__new = (char *) __builtin_alloca (__len); (char *) memcpy (__new, __old, __len); })); } while (0); arg_base = last_component (tmp_abns_); strip_trailing_slashes (arg_base); } while (0);
...
decode_preserve_arg (char const *arg, struct cp_options *x, _Bool on_off)
{
  ...
  static char const* const preserve_args[] =
    {
      "mode", "timestamps",
      "ownership", "links", "context", "all", ((void *)0)
    };
  extern int (* verify_function__ (void)) [(!!sizeof (struct { unsigned int verify_error_if_negative_size__: ((sizeof (preserve_args) / sizeof *(preserve_args)) == (sizeof (preserve_vals) / sizeof *(preserve_vals)) + 1) ? 1 : -1; }))];
 ...
 *comma++ = 0;
      ...
      val = ((preserve_vals) [__xargmatch_internal ("--preserve", s, preserve_args, (char const *) (preserve_vals), sizeof *(preserve_vals), argmatch_die)]);
      switch (val)
 {
 case PRESERVE_MODE:
...
main (int argc, char **argv)
{
 ...
 case REPLY_OPTION:
   x.interactive = ((reply_vals) [__xargmatch_internal ("--reply", optarg, reply_args, (char const *) (reply_vals), sizeof *(reply_vals), argmatch_die)]);

   error (0, 0,
   gettext ("the --reply option is deprecated; use -i or -f instead"));
   break;
 ...
 case 't':
   if (target_directory)
   ...
   else
     {
       struct stat st;
       if (stat (optarg, &st) != 0)
  error (1, (*__errno_location ()), gettext ("accessing %s"), quote (optarg));
 ...
 case GETOPT_VERSION_CHAR: version_etc (stdout, "cp", "GNU coreutils", "6.11", "Torbjorn Granlund", "David MacKenzie", "Jim Meyering", (char *) ((void *)0)); exit (0); break;;
