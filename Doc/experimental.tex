...
\figref{LinuxExample} depicts a patch made to the net/sunrpc/addr.c module in the Linux kernel SUNRPC implementation v2.6.32-rc6 which is aimed at removing an off-by-two array access out of bounds error in the original program. Although simple, this example shows two advantages of our tool, which are (i) the ability to analyze program without the need to run them and (ii) the ability to define fine-grained difference.\COMMENT{Previous methods would report equivalence for this patch, disregarding the array out of bounds access covered by the patch. Concrete methods~\cite{EnglerRamos11} would require symbolically running the two versions which would be problematic and time consuming considering the size of the system the code is embedded in.} Analyzing the correlated program for the two versions will produce the following result:
\\
\begin{tabular}{c}
$\sigma_1$:
\\ \hline
